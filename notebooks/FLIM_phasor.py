...
# def get_phasor_components2(flim_data, harmonic=1):
#     '''
#     Calculate phasor components G and S from the fourier transform
#     '''
#     import numpy as np
#     dc = np.sum(flim_data, axis=0)
#     # change the zeros to the img average
#     dc = np.where(dc != 0, dc, int(np.mean(dc)))
#     g = np.sum(flim_data[:,0,0] * np.cos(2*np.pi*frequency*1E6*time_array*1E-9), axis=0) )
#     g = g / dc
#     s = np.sum(flim_data[:,0,0] * np.sin(2*np.pi*frequency*1E6*time_array*1E-9), axis=0) )
#     s /= dc
#     return g, s, dc
